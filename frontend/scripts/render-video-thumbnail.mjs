...
if (!inputPath) throw new Error("A thumbnail input JSON path is required.");
...
const mimeTypes = { artwork: "image/png", displayFont: "font/ttf", utilityFont: "font/woff2", "brand-monogram": "image/png", "brand-lockup": "image/png", "brand-watermark": "image/png", "brand-vector": "image/svg+xml", "brand-approvalManifest": "application/json" };
...
for (const [key, asset] of Object.entries(snapshot.assets)) {
  if (!key.startsWith("audio-") && asset?.relativePath) {
...
const brand = snapshot.props.brand?.enabled ? {
  ...
  thumbnailMode: input.thumbnailMode !== false,
} : snapshot.props.brand;
const props = {
  ...snapshot.props,
...
try {
  ...
  emit({ stage: "rendering", progress: 0.25 });
  const composition = await selectComposition({ serveUrl, id: "AlbumLandscape", inputProps: props });
  const rawOutput = input.outputPath + ".raw.png";
  await renderStill({ composition, serveUrl, frame: renderFrame, output: rawOutput, inputProps: props, scale: 2 / 3 });
  await fs.rename(rawOutput, input.outputPath);
